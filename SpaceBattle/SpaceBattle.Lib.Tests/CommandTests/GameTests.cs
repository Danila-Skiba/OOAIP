...
namespace SpaceBattle.Lib.Tests
{
    public class GameTests
    {
        ...
        public void Execute_NullCommandInQueue_ThrowsInvalidOperationException()
        {
            ...
            var commandsQueue = (Queue<ICommand>)commandsField.GetValue(game)!;
        ...
        public void Execute_CommandsAreExecutedInFIFOOrder()
        {
            ...
            var cmd1 = new Mock<ICommand>();
            cmd1.Setup(c => c.Execute()).Callback(() => executionOrder.Add("first"));

            var cmd2 = new Mock<ICommand>();
            cmd2.Setup(c => c.Execute()).Callback(() => executionOrder.Add("second"));

            game.Receive(cmd1.Object);
            game.Receive(cmd2.Object);
            game.Execute();

            Assert.Equal(new List<string> { "first", "second" }, executionOrder);
        }
    }
}
